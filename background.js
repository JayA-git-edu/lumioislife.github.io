...
if (canvas) {
    ...
    function initRain() {
        rainDrops = [];
        const dropCount = Math.floor((canvas.width * canvas.height) / 5000);
        const accentColor = getComputedStyle(document.documentElement).getPropertyValue('--neon-blue').trim() || '#00d4ff';
    ...
    function initFireflies() {
        fireflies = [];
        const flyCount = Math.floor((canvas.width * canvas.height) / 20000);
        const accentColor = getComputedStyle(document.documentElement).getPropertyValue('--neon-blue').trim() || '#00d4ff';
        const accentColor2 = getComputedStyle(document.documentElement).getPropertyValue('--neon-purple').trim() || '#b026ff';
    ...
    function initMorphing() {
        ...
        for (let i = 0; i < shapeCount; i++) {
            morphingShapes.push({
                x: (canvas.width / (shapeCount + 1)) * (i + 1),
                y: canvas.height / 2,
    ...
    function drawSnow() {
        snowflakes.forEach(flake => {
            flake.rotation += flake.rotationSpeed * animationSpeed;
            ...
            flake.x += flake.speedX * animationSpeed;
            flake.y += flake.speedY * animationSpeed;
    ...
    function drawPlasma() {
        ...
        for (let y = 0; y < canvas.height; y++) {
            for (let x = 0; x < canvas.width; x++) {
                let value = 0;
                plasmaWaves.forEach((wave, i) => {
                    value += Math.sin((x * wave.frequency + y * wave.frequency * 0.5 + wave.offset + time * wave.speed * animationSpeed)) * 0.5;
                });
                
                const index = (y * canvas.width + x) * 4;
                const color = plasmaWaves[Math.floor(value * 2) % plasmaWaves.length].color;
                ...
                data[index + 1] = g;
                data[index + 2] = b;
                data[index + 3] = 50 + Math.abs(value) * 50;
            }
        }
    ...
    function drawMorphing() {
        morphingShapes.forEach(shape => {
            shape.angle += shape.speed * animationSpeed;
            const radius = shape.radius + Math.sin(shape.angle) * 10;
