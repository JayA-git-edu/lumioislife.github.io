...
function setupNavigationButtons() {
    ...
    function updateButtonStates() {
        ...
        // For back button: check if we have history
        if (backBtn) {
            // Simple heuristic: if history.length > 1, we might be able to go back
            // But we'll also track our own navigation
            const canGoBack = window.history.length > 1 || window.historyState.canGoBack;
            backBtn.disabled = !canGoBack;
        }
        
        // For forward button: track if we've gone back
        if (forwardBtn) {
            forwardBtn.disabled = !window.historyState.canGoForward;
        }
    }
    
    // Initial state - check if we can go back
    if (backBtn) {
        // On first load, typically can't go back
        const nav = performance.getEntriesByType('navigation')[0];
        if (nav && nav.type === 'navigate') {
            window.historyState.canGoBack = false;
        } else {
    ...
    // Track when we navigate forward (push new state)
    ...
    // Track when we navigate back
    window.addEventListener('popstate', (e) => {
        // When going back, we can now go forward
    ...
    setInterval(updateButtonStates, 300);
}
